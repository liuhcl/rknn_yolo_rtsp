...
{
    Mat img_bgr;
    cvtColor(ori_img, img_bgr, COLOR_RGB2BGR);
    ...
    { 
        ...
        rectangle(img_bgr, left_top, left_bottom, rec_color, 4, LINE_8); //框
        putText(img_bgr, det_result->name, name_result_pt, FONT_HERSHEY_PLAIN, 2, font_color, 2, LINE_8); //类别
        putText(img_bgr, score_result, score_result_pt, FONT_HERSHEY_PLAIN, 2, font_color, 2, LINE_8); //置信度
    }

    // imshow("rknn test", img_bgr);
    // waitKey(1); // 延时1毫秒,按q键退出
