...
const char *name = "my_mpp_enc_test";
mpp_main::mpp_main()
{
    int ret = 0;
    string output_url =  "rtmp://" + get_ipAdress() + "/live/stream";
    //string output_url = "output.flv";
...
int mpp_main::push_stream(uint8_t *ori_data)
{
    ...
    int ret = 0;
    int h264_buf_size;
    char *h264_buf = (char *)malloc(100000); //已知1280*720 普遍处于6000-8000
    AVPacket* pkt = NULL;
...
int mpp_main::ori_data_to_yuv420p(uint8_t* ori_data, uint8_t *yuv420p_buf)
{
    ...
    int src_buf_size = FRAME_BRG888_SIZE, dst_buf_size = FRAME_YUV420P_SIZE;
