...
MPP_RET MppEncoder::encode(const void* img, int img_len, char* dst, int *length)
{
	...
	*length = 0;
    ...
    // mpp默认设置为IPPPPP帧的模式， p->gop 为60帧
	//即每隔60帧写一次头信息
	if (countIdx_ % 60 == 0) { //(countIdx_ % 60)
		//countIdx_ = 0;
		int len = 0;
    ...
    do {
        ...
        if (packet) {
            ...
            p->pkt_eos = mpp_packet_get_eos(packet); //获取一个MppPacket结构体中的结束标志，也就是表示数据包是否是最后一个数据包
			...
			*length += len;
            ...
            if (mpp_packet_is_partition(packet)) { // MPP 包是否是一个分区包
				...
				//如果是结束标志，就将 p->frm_pkt_cnt 计数器清零，否则就将其加一
                p->frm_pkt_cnt = (eoi) ? (0) : (p->frm_pkt_cnt + 1); //用于记录一个编码帧中有多少个分区包
            }
