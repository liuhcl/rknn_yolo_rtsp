...
#define mpp_malloc(type, count)  \
    (type*)mpp_osal_malloc(__func__ , sizeof(type) * (count))

#define mpp_malloc_size(type, size)  \
    (type*)mpp_osal_malloc(__func__ , size)

#define mpp_calloc_size(type, size)  \
    (type*)mpp_osal_calloc(__func__ , size)

#define mpp_calloc(type, count)  \
    (type*)mpp_osal_calloc(__func__ , sizeof(type) * (count))

#define mpp_realloc(ptr, type, count) \
    (type*)mpp_osal_realloc(__func__ , ptr, sizeof(type) * (count))

#define mpp_free(ptr) \
    mpp_osal_free(__func__ , ptr)
